...
4. **Mapping is data, not code.** `expression_map.yaml` is loaded at startup, reloadable on `SIGHUP`. Adding a new emotion or burst is a config change. The `speech_emotion` event payload carries both the raw tag and the resolved fallback so consumers can use either.
...
- Common event envelope — `timestamp`, `schema_version` (currently 2), `source`, `correlation_id`, `payload`
