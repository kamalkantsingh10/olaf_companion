...
3. **Cloud-STT latency on Pi-class hardware (resolved).** The original concern — that cloud STT added an unacceptable round-trip to every utterance on a local personal agent — assumed CPU/GPU STT was free of network cost and that on-device deployment (Whisper + Hailo-8L) could match accuracy. With purpose-built LPU providers (Groq) the round-trip is ~150 ms p95, well inside NFR3; and on the accuracy side, the Hailo zoo caps at Whisper-Small (no accent fine-tunes), so on-device deployment now *regresses* accuracy vs cloud. The privacy footprint of cloud STT remains real and is acknowledged: audio of every utterance leaves the device. v1 ships with Groq as the default for accuracy; on-device Whisper stays available for operators who weight privacy over accuracy. *Reframed on 2026-05-12 — see `sprint-change-proposal-2026-05-12.md`.*
...
- **STT (configurable backend, cloud default)** — v1 default: Groq Whisper-Large-V3-Turbo via the openai-compatible audio endpoint. On-device `faster-whisper` retained as an opt-in offline alternative behind the same Protocol seam. *Reversed from on-device-only on 2026-05-12 — see Core Decision #3 below and `sprint-change-proposal-2026-05-12.md`.*
...
3. **Configurable STT backend with cloud as v1 default.** The pipeline ships with Groq Whisper-Large-V3-Turbo as the default STT (cloud, openai-compatible API surface). On-device `faster-whisper` is the opt-in offline alternative behind the same `STTBackend` Protocol seam (Story 1.4). The original "cloud STT explicitly excluded for v1" wording is reversed deliberately: (a) on-device deployment via Hailo regresses accuracy because the Hailo zoo caps at Whisper-Small with no Indian-accent fine-tune at that size; (b) Groq's LPU clears NFR3's ≤500 ms p95 budget, so the original "cloud round-trip" objection no longer holds; (c) the audio-leaves-device privacy footprint is acknowledged explicitly as a deliberate trade — operators who weight privacy over accuracy flip one TOML line. *Reversed on 2026-05-12 — see `sprint-change-proposal-2026-05-12.md` for full rationale and rejected alternatives.*
...
- **STT latency.** End-of-speech → transcript ready within **≤ 500ms** at p95 (NFR3). Budget applies regardless of backend; Groq's LPU clears it for the v1 cloud default.
...
- Audio I/O, wake-word, STT (Groq cloud default; on-device Whisper opt-in), Cartesia TTS, four-topic ROS 2 publish (mood / activity / speech_emotion / vocalization), activity FSM signaling
...
|-------|------|------------|
| 0 | Bare voice loop: Pipecat + STT (Groq cloud default; Whisper offline opt-in) + Cartesia TTS, push-to-talk, hardcoded prompt | Audio I/O, STT latency end-to-end, transport |
