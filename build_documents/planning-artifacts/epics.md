...
editsCompleted:
  ...
  - edit-2026-05-28-v2-expression-promotion-dr001-dr002-dr003-dr004
inputDocuments:
...
> **Status (2026-05-28):** **Superseded by Epic 6 (DR-001) post-v1.** The
> *cached-audio infrastructure* (`audio/cached.py`, `assets/audio/`, `manifest.json`
> + Stage 3 startup probe, `just regenerate-audio`) is reused by Epic 6.
> The *filler design* (`audio/filler.py` — timer-fired, random mood-keyed
> bucket, serialized in front of `tts.synthesize()`) is the part Epic 6 replaces
> with LLM-tag-selected function-bucketed openers + Cartesia overlap (FR54–FR58).
> **Story 5.5 remains v1's shipping perceived-latency-masking surface** until
> Epic 6 lands; the supersession note is here for audit history.
...
**When** the WS swap lands,
**Then** all existing tests continue to pass (the SSE→WS swap is internal to
`tts/cartesia.py`); a new unit test verifies `timestamps` capture round-trips
...
> **Status:** v2 backlog, **not currently scheduled.** DR-003 explicitly says
> pipeline change is zero now — the live-interaction dashboard tails the
> existing `voice-agent.log` (INFO). This story is the promotion path
> documented in DR-003 §Consequences (point 2 — "Log format is an unversioned
> contract"), to be taken **only if** log-format coupling between the pipeline
> and an external dashboard starts to bite (silent breaks from renamed events,
> field churn, etc.).

When taken, this story adds a parallel structlog sink at
`logs/events.jsonl` (or similar) carrying a *versioned* subset of events
that external consumers depend on — turn boundaries, transcript / response
events (subject to FR39's transcript-at-INFO redaction rules), latency
metrics, `speech_emotion` / `vocalization` / `activity` / `mood` summary
records (in addition to the wire publish — not replacing it). Adds a
`schema_version` field to the JSONL records (separate from the wire
`schema_version=3`). No new ROS 2 topics. No raw transcripts on the wire.
Dashboard / log-tail consumers migrate from `voice-agent.log` to
`events.jsonl` at their pace.

**Out of scope here:** the dashboard itself (separate consumer project per
DR-003); any new wire surface; cross-host log access (e.g. body's own log
on a separate Pi — that's a deployment concern outside this story).
