...
| Field | Type | Notes |
|---|---|---|
| `tag` | `str` | v1 set: `laughter`, `sigh`, `gasp`, `clears_throat` (audio bursts), `nod`, `shake` (gesture cues). **v2 (Epic 6 / Story 6.3 / DR-004) adds `emphasis`** as a 7th gesture-cue tag — the prosodic-stress head-cue, sourced from the Talker's emphasis marks × Cartesia word timestamps; `tts_supported: false` (the audio stress lives in the carrier word that Cartesia renders, not in a separate asset). As-built (Story 6.3): `tag` is an **open `str`** on the wire — the 7th value is an additive `expression_map.yaml` vocabulary entry, NOT a tightened `Literal` (open-set is the topic principle; tightening would break older subscribers). `schema_version` stays at 3. Its `audio_frame_id` carries the carrier word's audio offset in the form `seg-<N>-w-<start_ms>`. |
...
- **Schema stays at 3.** The `vocalization` tag set grows from 6 → 7
  (additive vocabulary entry in `expression_map.yaml`; the wire `tag` stays
  an open `str`, not a tightened `Literal` — as-built Story 6.3); subscribers
  updated to ignore unknown tags forward-compat survive an even-larger set,
  but a v1 embodiment built
